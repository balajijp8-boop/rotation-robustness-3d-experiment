...
git clone https://github.com/balajijp8-boop/rotation-robustness-3d-experiment
...
The dataset is generated by `code/generate_dataset.py` (no download needed). It consists of 11 HDF5 files totalling ~85 MB, saved to `data/synthetic_shapes/`:

| File | Description |
|------|-------------|
| `train.h5` | 2,000 aligned training samples |
| `test_rot000.h5` … `test_rot180.h5` | 7 Y-axis rotation conditions × 500 samples |
| `test_so3_t0.h5` … `test_so3_t2.h5` | 3 full SO(3) rotation seeds × 500 samples |

Run `python code/generate_dataset.py` to produce all files from scratch (~30 seconds on CPU).
